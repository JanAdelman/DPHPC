...
void naive_shift(std::vector<int> &input, const int h, MPI_Comm comm, const int world_rank, const int world_size, int *offsets_start, int local_length, int *offsets_end)
{
    ...
    else
    {
        ...
        for (int rank = world_rank; rank >= 0; rank--)
        { //Look at previous buckets

            if ((shift_start >= offsets_start[rank]) and (shift_start <= offsets_end[rank]))
            { //Start is within the offsets of this rank

                if (shift_end >= offsets_start[world_rank])
                { //End is also within bounds -> Only send to one porcessor
                ...
                else if (shift_end >= offsets_start[rank + 1])
                {
                    ...
                    MPI_Send(&input[offsets_end[rank] - shift_start + 1], local_length - (offsets_end[rank] - shift_start + 1), MPI_INT, rank + 1, offsets_start[rank + 1], comm); //MPI send end piece to rank where end is found; from start of bucket to calculated end
                    break;
                }
                else
                {
    ...
    //Recieving
    if (((offsets_end[world_size - 1] - offsets_end[world_rank]) >= h))
    { //No zeroes are needed

        if (local_length > h)
        { //Local shifting from direct neghbour
            MPI_Status status;
            int position;
            int number_amount;
        ...
        else
        {
            ...
            if (number_amount < local_length)
            { //If not enough recieve more
                MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                position = status.MPI_TAG;
                MPI_Get_count(&status, MPI_INT, &number_amount);
                MPI_Recv(&input[position - offsets_start[world_rank]], number_amount, MPI_INT, MPI_ANY_SOURCE, position, MPI_COMM_WORLD, &status); //Recieve start
